...
<div class="dashboard-wrapper">
    <div class="admin-card">
        <div class="admin-card__header">
            <h4 style="color: var(--admin-text);">Edit Guru</h4>
        </div>
        <div class="admin-card__body">
            <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="subject_ids[]" id="subject_ids" class="form-control select2-multiple @error('subject_ids') is-invalid @enderror" multiple>
                        @forelse($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ $teacher->subjects->contains('id', $subject->id) ? 'selected' : '' }}>
                                {{ $subject->name }}
                            </option>
                        @empty
                            <option disabled>Tidak ada mata pelajaran untuk sekolah ini</option>
                        @endforelse
                    </select>
                    @error('subject_ids')
                        <span class="text-danger">{{ $message }}</span>
...
$(document).ready(function() {
    ...
    // Store initially selected subjects
    let initialSelectedSubjects = [];
    $('#subject_ids').find('option:selected').each(function() {
        initialSelectedSubjects.push($(this).val());
    });
    console.log('Initially selected subjects:', initialSelectedSubjects);
            ...
            success: function(data) {
                ...
                if (!data || data.length === 0) {
                    ...
                    data.forEach(function(subject) {
                        const isSelected = initialSelectedSubjects.includes(subject.id.toString());
                        console.log('  - Subject ID ' + subject.id + ':', subject.name, isSelected ? '(SELECTED)' : '');
    ...
    $('#school_id').on('change', function() {
        ...
        // Reset initially selected subjects when school changes
